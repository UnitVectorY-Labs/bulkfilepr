...
func TestParseDefaultBranchFromJSON(t *testing.T) {
	tests := []struct {
		name        string
		json        string
		expected    string
		...
		{
			name:        "valid master branch",
			json:        `{"defaultBranchRef":{"name":"master"}}`,
			expected:    "master",
			expectError: false,
		},
		{
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			result, err := ParseDefaultBranchFromJSON(tt.json)
			if tt.expectError {
				if err == nil {
					t.Error("ParseDefaultBranchFromJSON() expected error, got nil")
				}
			} else {
				if err != nil {
					t.Errorf("ParseDefaultBranchFromJSON() error = %v", err)
				}
				if result != tt.expected {
					t.Errorf("ParseDefaultBranchFromJSON() = %q, want %q", result, tt.expected)
				}
			}
		})
	}
